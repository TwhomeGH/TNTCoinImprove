 ...
 */
export async function onGift(game, message) {
    ...
    if (actions && actions.length > 0) {
        actions.forEach((action) => executeGiftAction(game, action, coloredNickName, giftCount));
    }
 ...
 */
function handleGiftGoal(game, giftId, giftName, giftCount) {
 ...
 * @param {number} giftCount - The number of gifts sent.
 */
function executeGiftAction(game, action, coloredNickName, giftCount) {
